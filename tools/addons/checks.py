...
def check_method_input() -> str:
    ...
    """
    while (method := input(f"{F.GREEN}│   ├─── METHOD: {F.RESET}").lower()) not in [
        "http",
        "http-proxy",
        "slowloris",
        "slowloris-proxy",
        ...
        print(f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}Type a valid method!{F.RESET}")

    if method in ["syn-flood", "arp-spoof", "disconnect"] and os.getuid() != 0:
        print(
            f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}This attack needs Super User privileges!"
        )
        print(
            f"{F.GREEN}│   └───{F.MAGENTA} [!] {F.BLUE}Run: {F.GREEN}sudo {os.popen('which python').read()[:-1]} overload.py\n{F.RESET}"
        )
        sys.exit(1)
...
def check_number_input(x: str) -> int:
    ...
    while True:
        y = input(f"{F.GREEN}│   ├─── {x.upper()}: {F.RESET}")
        try:
            y = int(y)
            if y <= 0:
                raise ValueError
            ...
            print(
                f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}This value must be an integer number greater than zero!{F.RESET}"
            )
        else:
            return y
...
def check_http_target_input() -> str:
    ...
    """
    while True:
        target = input(f"{F.GREEN}│   ├─── URL: {F.RESET}")
        try:
            ...
            print(
                f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}Device is not connected to the internet!{F.RESET}"
            )
        except InvalidURL:
            print(f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}Invalid URL!{F.RESET}")
        else:
            return target
...
def check_local_target_input() -> str:
    ...
    while (target := input(f"{F.GREEN}│   ├─── IP: {F.RESET}")) not in hosts:
        print(
            f"{F.GREEN}│   ├───{F.MAGENTA} [!] {F.BLUE}Cannot connect to {F.CYAN}{target}{F.BLUE} on the local network!{F.RESET}"
        )
